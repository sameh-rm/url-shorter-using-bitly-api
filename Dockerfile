...
RUN apt-get update && apt-get install build-essential -y --no-install-recommends && apt-get clean  && rm -rf /var/lib/apt/lists/*
# RUN apt-get update && apt-get -y --no-install-recommends install make=3.75 && apt-get clean  && rm -rf /var/lib/apt/lists/*
